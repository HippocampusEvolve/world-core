 ...
 */
export const viewZ = (worldFov: number, fov: number): number =>
  Math.tan(THREE.MathUtils.degToRad(worldFov / 2)) / Math.tan(THREE.MathUtils.degToRad(fov / 2))
...
export interface ViewModelOptions {
  ...
  key?: { color: THREE.ColorRepresentation; intensity: number }
  fill?: { sky: THREE.ColorRepresentation; ground: THREE.ColorRepresentation; intensity: number }
...
export class ViewModel {
  ...
  swayPitch = 0
  breathT = 0
  dip: Spring = { x: 0, v: 0 } // просадка при приземлении (см. spring.ts)
