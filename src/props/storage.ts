...
import { extrude, fillet, frameOf, mesh, merge, boxGeo, pipe, revolve, type P3 } from './shapes.js'
...
export function locker({ w = 0.5, d = 0.5, h = 1.8, open = false, sealed = false, hangers, mats }: LockerOptions = {}): Locker {
  ...
  for (const y of [doorH / 2 - 0.12, -doorH / 2 + 0.1]) {
    for (let i = 0; i < 3; i++) louvre.push(boxGeo(0.14, 0.008, 0.004, GAP + dw / 2, y - i * 0.022, 0.004))
  }
