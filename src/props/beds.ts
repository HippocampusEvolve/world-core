...
import { extrude, fillet, frameOf, mesh, pipe, slab, type P3 } from './shapes.js'

type Sized = { group: THREE.Group; w: number; d: number; h: number }

/* --------------------------------- двухъярусная --------------------------------- */

export type BunkOptions = { w?: number; l?: number; h?: number; mats?: Mats }
 ...
 */
export function bunk({ w = 0.9, l = 2.0, h = 1.8, mats }: BunkOptions = {}): Bunk {
  ...
  const x0 = w / 2 - P
  const z0 = l / 2 - P
  for (const sx of [-1, 1]) for (const sz of [-1, 1]) g.add(mesh('bunk-post', pipe([[sx * x0, 0, sz * z0], [sx * x0, h, sz * z0]], P, 8), m('paint')))
  const levels: number[] = []
  const BT = 0.02
  for (const y of [0.33, 1.23]) {
    // продольные царги по стойкам, поперечные на 4 см выше
    for (const sx of [-1, 1]) g.add(mesh('bunk-rail', pipe([[sx * x0, y, -(z0 - P)], [sx * x0, y, z0 - P]], Rr, 8), m('paint')))
    for (const sz of [-1, 1]) g.add(mesh('bunk-end-rail', pipe([[-(x0 - P), y + 0.04, sz * z0], [x0 - P, y + 0.04, sz * z0]], Rr, 8), m('paint')))
  ...
  for (const sz of [-1, 1]) {
    for (const y of [0.66, h - 0.06]) g.add(mesh('bunk-end-bar', pipe([[-(x0 - P), y, sz * z0], [x0 - P, y, sz * z0]], Rr, 8), m('paint')))
  }
  g.add(mesh('bunk-guard', pipe([[x0, 1.5, -(z0 - P)], [x0, 1.5, z0 - P]], Rr, 8), m('paint')))
  return { group: g, w, d: l, h, levels }
}

/* ------------------------------ одинарная застеленная ------------------------------ */

export type CotOptions = { w?: number; l?: number; mats?: Mats }
export type Cot = Sized & { top: number; mattress: THREE.Mesh; blanket: THREE.Mesh; pillow: THREE.Mesh }
 ...
 */
export function cot({ w = 0.9, l = 2.0, mats }: CotOptions = {}): Cot {
  const g = new THREE.Group()
  g.name = 'cot'
  const m = roleMats(mats)
  const P = 0.02
  ...
  const ry = 0.38
  for (const sx of [-1, 1]) g.add(mesh('cot-rail', pipe([[sx * x0, ry, -(z0 - P)], [sx * x0, ry, z0 - P]], Rr, 8), m('paint')))
  ...
  const pc = (cx: number, cy: number, a0: number) => {
    for (let k = 0; k <= 3; k++) {
      const a = a0 + (k / 3) * (Math.PI / 2)
...
export type RolledMattressOptions = { l?: number; d?: number; mats?: Mats }
 ...
 */
export function rolledMattress({ l = 0.9, d = 0.32, mats }: RolledMattressOptions = {}): RolledMattress {
  const g = new THREE.Group()
...
/* ------------------------------ сложенная раскладушка ------------------------------ */

export type FoldedCotOptions = { mats?: Mats }
 ...
 */
export function foldedCot({ mats }: FoldedCotOptions = {}): FoldedCot {
  const g = new THREE.Group()
  g.name = 'folded-cot'
  const m = roleMats(mats)
  const r = 0.012
  const W = 0.65
  const H = 0.95
  const loop = (z: number): THREE.BufferGeometry => {
    const pts: P3[] = [
      [-W / 2, r, z],
      [W / 2, r, z],
      [W / 2, r + H, z],
