...
export type Table = { group: THREE.Group; w: number; d: number; h: number }

export function table({ w = 1.15, d = 0.75, h = 0.755, mats }: TableOptions = {}): Table {
...
export type StoolOptions = { r?: number; h?: number; mats?: Mats }
export type Stool = { group: THREE.Group; r: number; h: number }

export function stool({ r = 0.19, h = 0.45, mats }: StoolOptions = {}): Stool {
  const g = new THREE.Group()
  g.name = 'stool'
