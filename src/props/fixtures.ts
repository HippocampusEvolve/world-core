...
import { fillet, frameOf, mesh, pipe, pivot, revolve, slab, type P3 } from './shapes.js'

type Sized = { group: THREE.Group; w: number; d: number; h: number }

/* ------------------------------ доска с крючками ------------------------------ */

export type CoatBoardOptions = { hooks?: number; w?: number; mats?: Mats }
export type CoatBoard = Sized & {
  /**
   * Точки, где висит петля: на них ставят начало `coat`. В координатах доски;
   * `z` точки - на сколько она выступает от стены (`reach` у плаща).
   */
  hooks: THREE.Vector3[]
 ...
 */
export function coatBoard({ hooks = 5, w = 1.3, mats }: CoatBoardOptions = {}): CoatBoard {
...
/* ------------------------------- зеркало под простынёй ------------------------------- */

export type CoveredMirrorOptions = { w?: number; h?: number; mats?: Mats }
export type CoveredMirror = Sized & { sheet: THREE.Mesh; glass: THREE.Mesh }
 ...
 */
export function coveredMirror({ w = 0.5, h = 0.7, mats }: CoveredMirrorOptions = {}): CoveredMirror {
  const g = new THREE.Group()
  g.name = 'covered-mirror'
  const m = roleMats(mats)
  const F = 0.03 // ширина планки рамы
  const FD = 0.025 // глубина рамы
  for (const sy of [-1, 1]) g.add(boxMesh('mirror-frame', w, F, FD, m('wood'), 0, sy * (h / 2 - F / 2), FD / 2, 2, 'x'))
  for (const sx of [-1, 1]) g.add(boxMesh('mirror-frame', F, h - 2 * F, FD, m('wood'), sx * (w / 2 - F / 2), 0, FD / 2, 2, 'y'))
  ...
  const N = 12
  for (let k = 1; k <= N; k++) path.push([FD + mid, h / 2 - (k / N) * (h / 2 + drop)])
  const top = 7 // точек до начала свеса
  const f = (u: number, v: number): P3 => {
...
export type ExtinguisherOptions = { mats?: Mats }
export type Extinguisher = Sized & { body: THREE.Mesh }
 ...
 */
export function extinguisher({ mats }: ExtinguisherOptions = {}): Extinguisher {
  const g = new THREE.Group()
  g.name = 'extinguisher'
...
export type CableTrayOptions = { length?: number; cables?: number; mats?: Mats }
...
export type WashbasinOptions = { w?: number; d?: number; h?: number; mats?: Mats }
export type Washbasin = Sized & { rim: number; bowl: THREE.Mesh; spout: THREE.Vector3 }
 ...
 */
export function washbasin({ w = 0.55, d = 0.42, h = 0.85, mats }: WashbasinOptions = {}): Washbasin {
  const g = new THREE.Group()
  g.name = 'washbasin'
  const m = roleMats(mats)
  const a = w / 2
  const b = d / 2
  const k = (x: number) => x // доли под размеры по умолчанию
  const s = (x: number, y: number, z: number): [number, number, number] => [k(x) * (a / 0.275), y, z * (b / 0.21)]
  const bowl = mesh(
    'washbasin-bowl',
    revolve(
      [
...
export type WallClock = Sized & { hands: { hour: THREE.Group; minute: THREE.Group; second: THREE.Group }; face: THREE.Mesh }
 ...
 */
export function wallClock({ d = 0.3, time = 0, mats }: WallClockOptions = {}): WallClock {
  const g = new THREE.Group()
  g.name = 'wall-clock'
  const m = roleMats(mats)
  const k = d / 0.3
  ...
  const hand = (name: string, moving: string, len: number, wid: number, z: number, t: number, angle: number) => {
...
export type PhotoFrameOptions = { w?: number; h?: number; mats?: Mats }
export type PhotoFrame = Sized & { photo: THREE.Mesh }
 ...
 */
export function photoFrame({ w = 0.3, h = 0.2, mats }: PhotoFrameOptions = {}): PhotoFrame {
  const g = new THREE.Group()
  g.name = 'photo-frame'
  const m = roleMats(mats)
  const F = 0.02
...
export type NightLampOptions = { mats?: Mats }
export type NightLamp = Sized & { bulb: THREE.Mesh; shade: THREE.Mesh }
 ...
 */
export function nightLamp({ mats }: NightLampOptions = {}): NightLamp {
  const g = new THREE.Group()
  g.name = 'night-lamp'
  const m = roleMats(mats)
  ...
  const bulb = mesh(
    'lamp-bulb',
    revolve([[[0, 0.18], [0.01, 0.18]], [[0.01, 0.18], [0.011, 0.19], [0.025, 0.205], [0.026, 0.22], [0.018, 0.24], [0, 0.248]]], 14),
    m('bulb'),
  )
  g.add(bulb)
  const shade = mesh(
