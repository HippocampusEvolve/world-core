...
import { extrude, frameOf, loft, mesh, type Section } from './shapes.js'
...
function table(rows: [number, number][], x: number): number {
 ...
 */
export function coat({ length = 1.25, hood = true, cape = false, width, reach = 0.064, seed = 1, mats }: CoatOptions = {}): Coat {
  ...
  const top: [number, number, number][] = hood
    ? [
  ...
  const widthRows: [number, number][] = cape
    ? [
        [0, 0.15],
        [0.06, 0.21],
        [0.14, 0.25],
  ...
  for (const row of widthRows) row[1] = Math.max(Math.min(row[1], 0.11), row[1] * k)
  ...
  const zAt = (d: number, back: number) => Math.max(wall + back, lerp(0, wall + back, ss(d0, d0 + 0.35, d)))

  const sections: Section[] = [{ y: -d0, z: 0, a: 0, b: 0 }]
...
export type BootsOptions = { mats?: Mats }
export type Boots = Sized & { left: THREE.Mesh; right: THREE.Mesh }
 ...
 */
export function boots({ mats }: BootsOptions = {}): Boots {
  const g = new THREE.Group()
  g.name = 'boots'
  const m = roleMats(mats)
  const BEV = 0.015
  ...
  const prof = side.map(([u, v]) => [u * ku, v] as [number, number])
  const one = (name: string, x: number) => {
  ...
  for (const b of [left, right]) b.position.z -= (box.min.z + box.max.z) / 2
