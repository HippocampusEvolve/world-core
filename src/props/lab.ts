...
import { boxGeo, extrude, frameOf, merge, mesh, pipe, place, revolve, slab, type P3 } from './shapes.js'
...
export type PlantRackOptions = { w?: number; d?: number; h?: number; shelves?: number; mats?: Mats }
 ...
 */
export function plantRack({ w = 2.0, d = 0.5, h = 1.8, shelves = 3, mats }: PlantRackOptions = {}): PlantRack {
  ...
  const px = w / 2 - S / 2
  const pz = d / 2 - S / 2
  for (const sx of [-1, 1]) for (const sz of [-1, 1]) g.add(boxMesh('rack-post', S, h, S, m('paint'), sx * px, h / 2, sz * pz, 1, 'y'))
  const inX = w - 2 * S
  const y0 = 0.15
  ...
  for (let i = 0; i <= shelves; i++) {
    ...
    for (const sz of [-1, 1]) g.add(boxMesh('rack-beam', inX, RT, RT, m('paint'), 0, yb - RT / 2, sz * pz, 1, 'x'))
    g.add(boxMesh(i < shelves ? 'rack-shelf' : 'rack-roof', inX, PT, 2 * pz, m('steel'), 0, yb + PT / 2, 0, 1, 'x'))
    if (i < shelves) tops.push(yb + PT)
    if (i > 0) {
...
export type TomatoPlantOptions = { fruits?: number; seed?: number; mats?: Mats }
export type TomatoPlant = Sized & { fruits: THREE.Mesh; leaves: THREE.Mesh }
 ...
 */
export function tomatoPlant({ fruits = 6, seed = 1, mats }: TomatoPlantOptions = {}): TomatoPlant {
  const g = new THREE.Group()
  g.name = 'tomato-plant'
  const m = roleMats(mats)
  const rnd = rng(seed * 7919 + 13)
  const S = 12
  const wallAt = (y: number) => 0.056 + ((y - 0.01) / 0.13) * 0.019
  g.add(
    mesh(
      'tomato-pot',
      revolve(
  ...
  const stemAt = (y: number): THREE.Vector3 => {
  ...
  const fruitSpots: THREE.Vector3[] = []
  ...
  heights.forEach((y, i) => {
    ...
    const br = 0.0028
    g.add(mesh('tomato-branch', pipe([base.toArray() as P3, mid.toArray() as P3, end.toArray() as P3], br, 5), m('leaf')))
...
export type JarOptions = { h?: number; fill?: number; lid?: boolean; mats?: Mats }
 ...
 */
export function jar({ h = 0.2, fill = 0.6, lid = true, mats }: JarOptions = {}): Jar {
  const g = new THREE.Group()
  g.name = 'jar'
  const m = roleMats(mats)
  const S = 16
  ...
  if (lid) g.add(cylMesh('jar-lid', 0.038, 0.038, h - neck, S, m('steel'), 0, neck + (h - neck) / 2, 0, 4))
  return { group: g, ...frameOf(g) }
}

/* --------------------------------- микроскоп --------------------------------- */

export type MicroscopeOptions = { mats?: Mats }
 ...
 */
export function microscope({ mats }: MicroscopeOptions = {}): Microscope {
  const g = new THREE.Group()
  g.name = 'microscope'
  ...
  const along = (s: number) => foot.clone().addScaledVector(axis, s)
  ...
  const arm: P3[] = [[0, 0.14, -0.06], [0, 0.2, -0.08], pre.toArray() as P3, E.toArray() as P3]
  g.add(mesh('microscope-arm', pipe(arm, 0.014, 8), m('paint')))
  const cyl = (name: string, r: number, s0: number, s1: number, role: 'steel' | 'paint' | 'glass') => {
...
export type TagBoxOptions = { tags?: number; seed?: number; mats?: Mats }
export type TagBox = Sized & { tags: THREE.Mesh }
 ...
 */
export function tagBox({ tags = 16, seed = 1, mats }: TagBoxOptions = {}): TagBox {
  const g = new THREE.Group()
  g.name = 'tag-box'
  const m = roleMats(mats)
...
export type ScalesOptions = { mats?: Mats }
export type Scales = Sized & { weight: THREE.Group }
 ...
 */
export function scales({ mats }: ScalesOptions = {}): Scales {
  const g = new THREE.Group()
  g.name = 'scales'
  const m = roleMats(mats)
...
export type BandageOptions = { mats?: Mats }
export type Bandage = Sized & { roll: THREE.Mesh; tail: THREE.Mesh }
 ...
 */
export function bandage({ mats }: BandageOptions = {}): Bandage {
  const g = new THREE.Group()
  g.name = 'bandage'
  const m = roleMats(mats)
  const R = 0.05
...
export type ScissorsOptions = { mats?: Mats }
 ...
 */
export function scissors({ mats }: ScissorsOptions = {}): Scissors {
  const g = new THREE.Group()
  g.name = 'scissors'
  const m = roleMats(mats)
  const T = 0.0022
  ...
  const half = (name: string, side: number, y: number) => {
    const blade = extrude(side > 0 ? outline : outline.map(([x, v]) => [x, -v] as [number, number]), T)
  ...
  for (const o of [lower, upper, screw]) o.position.x -= cx
  return { group: g, ...frameOf(g) }
}
