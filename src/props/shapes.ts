...
const V = (p: P3 | THREE.Vector3): THREE.Vector3 => (Array.isArray(p) ? new THREE.Vector3(p[0], p[1], p[2]) : p.clone())
 ...
 */
export function frameOf(g: THREE.Object3D): { w: number; d: number; h: number } {
...
export function pivot(name: string, x = 0, y = 0, z = 0, moving?: string): THREE.Group {
  const g = new THREE.Group()
  g.name = name
  g.position.set(x, y, z)
  if (moving) g.userData.moving = moving
  return g
}
 ...
 */
export function orient(geo: THREE.BufferGeometry): THREE.BufferGeometry {
...
function build(pos: number[], uv: number[], idx: number[], groups?: [number, number, number][]): THREE.BufferGeometry {
  const g = new THREE.BufferGeometry()
  g.setAttribute('position', new THREE.Float32BufferAttribute(pos, 3))
  g.setAttribute('uv', new THREE.Float32BufferAttribute(uv, 2))
  g.setIndex(idx)
  if (groups) for (const [s, n, m] of groups) g.addGroup(s, n, m)
  orient(g)
  return g
}

/** Сгладить нормали там, где швы развёртки разрезали вершины, и оставить рёбра острыми. */
function shade(g: THREE.BufferGeometry, crease = Math.PI / 4): THREE.BufferGeometry {
 ...
 * стоит треугольников.
 */
export function fillet(points: P3[], r: number, steps = 3, only?: number[]): P3[] {
 ...
 */
export function pipe(points: P3[], r: number | ((t: number) => number), radial = 8, loop = false): THREE.BufferGeometry {
  ...
  for (let i = 0; i < segs; i++) dir.push(P[(i + 1) % n].clone().sub(P[i]).normalize())
  ...
  const frames: THREE.Vector3[] = [helper.clone().cross(d0).normalize()]
  ...
  const ring = (i: number, k: number) => rings[i] + k
...
export type Run = { pts: ProfilePoint[]; mat?: number; disc?: boolean }
 ...
 */
export function revolve(runs: (Run | ProfilePoint[])[], segments = 16, power = 2): THREE.BufferGeometry {
  const pos: number[] = []
  const uv: number[] = []
  const idx: number[] = []
  const groups: [number, number, number][] = []
  const e = 2 / power
  const sec = (a: number): [number, number] => {
    const c = Math.cos(a)
    const s = Math.sin(a)
    return [Math.sign(c) * Math.abs(c) ** e, Math.sign(s) * Math.abs(s) ** e]
  }
  runs.forEach((raw, ri) => {
    const run: Run = Array.isArray(raw) ? { pts: raw } : raw
    ...
    pts.forEach((p, j) => {
      if (j > 0) v += Math.hypot(p.x - pts[j - 1].x, p.y - pts[j - 1].y)
      const flatUV = (x: number, z: number): [number, number] =>
        run.disc ? [0.5 + x / (2 * rmax), 0.5 + z / (2 * rmax)] : [0, 0]
      ...
      for (let k = 0; k <= segments; k++) {
        ...
        pos.push(x, p.y, z)
        if (run.disc) uv.push(...flatUV(x, z))
        else uv.push(a * (p.x + p.z) * 0.5, v)
      }
      rows.push(row)
    })
    for (let j = 0; j < rows.length - 1; j++) {
      ...
      const Bn = rows[j + 1]
      for (let k = 0; k < segments; k++) {
        if (typeof A === 'number' && typeof Bn === 'number') continue
        if (typeof A === 'number') idx.push(A, (Bn as number[])[k + 1], (Bn as number[])[k])
        else if (typeof Bn === 'number') idx.push(A[k], A[k + 1], Bn)
        else idx.push(A[k], A[k + 1], Bn[k + 1], A[k], Bn[k + 1], Bn[k])
      }
    }
 ...
 */
export function slab(nu: number, nv: number, f: (u: number, v: number) => P3, t: number): THREE.BufferGeometry {
  const P: THREE.Vector3[][] = []
  for (let i = 0; i <= nu; i++) {
    P.push([])
    for (let j = 0; j <= nv; j++) P[i].push(V(f(i / nu, j / nv)))
  }
  ...
  const U: number[] = [0]
  for (let i = 1; i <= nu; i++) U.push(U[i - 1] + P[i][nv >> 1].distanceTo(P[i - 1][nv >> 1]))
  const W: number[] = [0]
  for (let j = 1; j <= nv; j++) W.push(W[j - 1] + P[nu >> 1][j].distanceTo(P[nu >> 1][j - 1]))
  ...
  const side = (sgn: number) => {
    ...
    return (i: number, j: number) => base + i * (nv + 1) + j
  }
  const F = side(1)
  const Bk = side(-1)
  for (let i = 0; i < nu; i++) {
  ...
  for (let i = 0; i < nu; i++) edge.push([i, 0])
  for (let j = 0; j < nv; j++) edge.push([nu, j])
  for (let i = nu; i > 0; i--) edge.push([i, nv])
  for (let j = nv; j > 0; j--) edge.push([0, j])
  let run = 0
 ...
 */
export function extrude(outline: [number, number][], depth: number, bevel = 0, curveSegments = 4): THREE.BufferGeometry {
...
function weld(g: THREE.BufferGeometry): THREE.BufferGeometry {
  ...
  const uvA = src.attributes.uv as THREE.BufferAttribute
  const P: number[] = []
  const UVs: number[] = []
  const I: number[] = []
  const a = new THREE.Vector3()
...
export function merge(geos: THREE.BufferGeometry[]): THREE.BufferGeometry {
  ...
  if (!out) throw new Error('merge: геометрии с разными атрибутами')
  return out
}
...
export function boxGeo(w: number, h: number, d: number, x = 0, y = 0, z = 0): THREE.BufferGeometry {
