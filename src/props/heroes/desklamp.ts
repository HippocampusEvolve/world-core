...
export type DeskLampOptions = { mats?: Mats }
...
export function deskLamp({ mats }: DeskLampOptions = {}): DeskLamp {
  ...
  const shell = prismGeo(
    [...arc(0, 0, SHADE_R, 0, Math.PI, 24), ...arc(0, 0, SHADE_R - GLASS, Math.PI, 0, 24)],
    'x',
    -SHADE_L / 2,
    SHADE_L / 2,
  )
  const ends = [-1, 1].map((s) =>
    prismGeo(arc(0, 0, SHADE_R - GLASS, 0, Math.PI, 24), 'x', s < 0 ? -SHADE_L / 2 : SHADE_L / 2 - GLASS, s < 0 ? -SHADE_L / 2 + GLASS : SHADE_L / 2),
  )
