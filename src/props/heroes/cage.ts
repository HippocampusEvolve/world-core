 ...
 */
function lampHead(look: ReturnType<typeof looks>, name: string): THREE.Group {
  ...
  const c = -0.05
  const bulbProfile: [number, number][] = [
    [0, -0.065],
    [0.012, -0.062],
    [0.022, -0.054],
    [0.028, -0.042],
  ...
  const wires: THREE.BufferGeometry[] = [torusGeo(COLLAR, WIRE, 4, 20, 'y', 0, 0.006, 0)]
  for (let k = 0; k < 6; k++) {
...
export function cageLamp({ mount = 'hang', cord = 0.5, mats }: CageLampOptions = {}): CageLamp {
