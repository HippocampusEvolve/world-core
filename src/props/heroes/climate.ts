...
export type ClimateConsoleOptions = { w?: number; d?: number; h?: number; mats?: Mats }
...
export function climateConsole({ w = 1.2, d = 0.5, h = 1.2, mats }: ClimateConsoleOptions = {}): ClimateConsole {
  ...
  DXS.forEach((x, i) => {
    ...
    n.add(
      merged(
        `needle-${i + 1}`,
        [spanGeo(-0.0012, 0.0012, 0.0005, 0.002, -0.042, 0.006), cylGeo(0.005, 0.005, 0.003, 6, 'y', 0, 0.002, 0)],
        look('plastic'),
      ),
    )
    mount.add(n)
...
/** Точка периметра ящика: база на верхнем ребре и нормаль наружу в плане. */
type Around = { bx: number; bz: number; nx: number; nz: number; fold: number; along: number }

function perimeter(w: number, d: number): Around[] {
  const X = w / 2
  const Z = d / 2
...
export function tarp({ w = 1.2, d = 0.5, h = 1.2, mats }: TarpOptions = {}): Tarp {
  const look = looks(mats)
  const g = new THREE.Group()
  g.name = 'tarp'
  ...
  const wrinkle = (x: number, z: number) =>
    WRINKLE * Math.cos((Math.PI * x) / w) * Math.cos((Math.PI * z) / d) * (0.75 + 0.25 * Math.cos((6 * Math.PI * x) / w))
  ...
  const tri = (a: number, b: number, c: number) => {
    const A = at(a)
    if (A.distanceToSquared(at(b)) < 1e-14 || A.distanceToSquared(at(c)) < 1e-14 || at(b).distanceToSquared(at(c)) < 1e-14) return
    idx.push(a, b, c)
  }

  /** Кольца одной поверхности на отступе `c`; возвращает начало каждого кольца. */
  const surface = (c: number): number[] => {
    const starts: number[] = []
    const push = (f: (p: Around) => [number, number, number], v: number, scale: number) => {
  ...
  const band = (rows: number[], flip: boolean) => {
