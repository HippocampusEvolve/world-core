 ...
 */
function shroud(len: number, floorY: number, x0: number): THREE.BufferGeometry {
  ...
  const uv: number[] = []
  const idx: number[] = []
  const ring = (t: number, a: number, b: number) => {
    const x = x0 + t * len
    for (let i = 0; i <= N; i++) {
  ...
  for (const [t, a, b] of prof) ring(t, a, b)
  const R = N + 1
...
export function pod({ l = 2.1, w = 0.8, open = false, occupied = true, mats }: PodOptions = {}): Pod {
