...
export function carcass(o: CarcassOptions): Carcass {
  const T = o.T ?? 0.018
  const base = o.base ?? 0
  ...
  if (top) g.add(boxMesh(`${name}-top`, w, T, d, o.body, 0, h - T / 2, 0, 1, 'x'))
  const wi = w - 2 * T
  // дно и перемычки - между боковинами и перед задником
  const rail = (y: number, label: string) =>
    g.add(boxMesh(`${name}-${label}`, wi, T, d - T, o.body, 0, y + T / 2, T / 2, 1, 'x'))
  rail(base, 'bottom')
  ...
  o.slots.forEach((s, i) => {
    ...
    if (s.kind === 'drawer') drawers.push(drawer(g, `${name}-drawer`, wi, sh, d, T, yc, o, drawers.length))
    if (s.kind === 'door') doors.push(door(g, `${name}-door`, wi, sh, d, T, yc, s.hinge ?? 'left', o, 'door'))
    if (s.kind === 'doors') {
    ...
    shelves.push(y0)
    y = y0 - T
    if (i < o.slots.length - 1) rail(y, 'rail')
  })
...
function handle(p: THREE.Group, o: CarcassOptions, x: number, y: number): void {
