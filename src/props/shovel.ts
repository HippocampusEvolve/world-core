...
export function shovel(mats: { wood: THREE.Material; steel: THREE.Material }): THREE.Group {
  const pieces: Record<'wood' | 'steel', THREE.BufferGeometry[]> = { wood: [], steel: [] }
  const add = (role: 'wood' | 'steel', g: THREE.BufferGeometry, x: number, y: number, z: number, rz = 0) => {
    if (rz) g.rotateZ(rz)
    g.translate(x, y, z)
  ...
  for (let row = 0; row < rows; row++) for (let col = 0; col < columns; col++) {
    ...
    quad(a, d, c, b); quad(a + layer, b + layer, c + layer, d + layer)
  }
  for (let col = 0; col < columns; col++) {
  ...
  const root = new THREE.Group(); root.name = 'Procedural shovel'
  for (const role of ['steel', 'wood'] as const) {
    const geometry = mergeGeometries(pieces[role])!
    for (const part of pieces[role]) part.dispose()
    const mesh = new THREE.Mesh(geometry, mats[role]); mesh.name = `shovel/${role}`
