...
export const NATURE_VOICES: NatureVoice[]=['songbird','raven','owl','fox','deer','bough','snowfall','ice']
const TAU=Math.PI*2
...
export function natureSamples(kind:NatureVoice,seed=1,sampleRate=24000) {
  ...
  for(let i=0;i<data.length;i++) {
    ...
    if(kind==='songbird') {
      ...
      for(let k=0;k<4;k++) {
        ...
        if(u>0)value+=Math.sin(TAU*(710+k*293)*variation*u)*Math.exp(-u*6)*(1-Math.exp(-u*100))*0.055
      }
  ...
  let sum=0,weight=0
  for(let i=0;i<data.length;i++){const w=Math.sin(Math.PI*i/(data.length-1))**2;sum+=data[i];weight+=w}
  const correction=sum/weight
  let peak=0
  for(let i=0;i<data.length;i++) {
    data[i]-=correction*Math.sin(Math.PI*i/(data.length-1))**2;peak=Math.max(peak,Math.abs(data[i]))
  }
  if(peak>0.32)for(let i=0;i<data.length;i++)data[i]*=0.32/peak
...
export interface SoundscapeState {position:Position;direction:Position;shelter?:number;storm?:number;active?:boolean}

export function createNatureSoundscape(getBus:()=>Bus|null,{seed=37,alpine=false}={}) {
  const rand=randomOf(seed),cache=new Map<string,AudioBuffer>(),playing=new Set<AudioBufferSourceNode>()
  let last:SoundscapeState|null=null,timer=3,age=0,count=0,disposed=false,cachedContext:AudioContext|null=null
  function play(kind:NatureVoice,position:Position,level=1) {
    if(disposed||!last||last.active===false)return false
    const bus=getBus();if(!bus||bus.ctx.state!=='running'||playing.size>=5)return false
    ...
    if(ctx!==cachedContext){cache.clear();cachedContext=ctx}
    ...
    if(!cache.has(key)) {
      ...
      buffer.copyToChannel(samples.data,0);cache.set(key,buffer)
    }
    const source=ctx.createBufferSource(),gain=ctx.createGain(),pan=ctx.createStereoPanner(),lp=ctx.createBiquadFilter()
    source.buffer=cache.get(key)!;source.playbackRate.value=0.97+rand()*0.06
    ...
    lp.type='lowpass';lp.frequency.value=Math.max(550,5800-distance*58-shelter*3500)
    ...
    playing.add(source);source.start();count++;return true
  }
  return {play,get events(){return count},get activeVoices(){return playing.size},get cachedVoices(){return cache.size},
    update(dt:number,state:SoundscapeState) {
      last=state
      if(disposed||state.active===false||!getBus())return
      const step=Math.max(0,Math.min(0.1,dt));timer-=step;age+=step
      if(timer>0)return
      const storm=state.storm??0,shelter=state.shelter??0
      timer=alpine?10+rand()*13:7+rand()*12
      if(shelter>0.75||storm>0.8)return
      const a=rand()*TAU,r=8+rand()*20,p=state.position
      const choices:NatureVoice[]=alpine?['raven','ice','snowfall','bough']:['songbird','bough','snowfall','owl']
    ...
    },dispose(){disposed=true;for(const source of playing){try{source.stop()}catch{}}playing.clear();cache.clear()}}
}
