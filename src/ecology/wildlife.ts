...
import { createAnimal, type AnimalKind } from './animals.js'
...
export interface WildlifeWeather {active?:boolean;shelter?:number;storm?:number;reducedMotion?:boolean;direction?:Location}
...
export function createWildlife(habitat:Habitat) {
  const root=new THREE.Group();root.name='forest-wildlife'
  const seed=habitat.seed??719, rand=randomOf(seed), tau=Math.PI*2
  const animals=(habitat.kinds??['fox','deer','raven'] as AnimalKind[]).map((kind,i)=> {
    const model=createAnimal(kind,seed+i*47);model.root.visible=false;root.add(model.root)
  ...
  function spawn(a:typeof animals[number],p:Location,direction?:Location) {
    for(let n=0;n<30;n++) {
      ...
      if(dist<13||dist>65)continue
      // New encounters enter from outside the visible forward hemisphere.
      if(direction&&((x-p.x)*direction.x+(z-p.z)*direction.z)>dist*0.15)continue
      const y=ground(x,z)
      if(y===null||Math.abs(y-p.y)>9)continue
      let safe=true
      for(const [dx,dz] of [[1,0],[-1,0],[0,1],[0,-1]]) {
        const gy=ground(x+dx,z+dz);if(gy===null||Math.abs(gy-y)>0.35)safe=false
      }
      if(!safe)continue
      a.x=x;a.z=z;a.y=y;a.homeX=x;a.homeZ=z;a.yaw=rand()*tau;a.targetYaw=a.yaw
      a.active=true;a.age=0;a.timer=3+rand()*5;a.state='watch';a.speed=0;a.flown=0
      a.previousX=x;a.previousY=y;a.previousZ=z;a.previousYaw=a.yaw;a.graze=0;a.alert=0
      a.model.root.visible=true;return true
    }
    return false
  }
  function step(dt:number,p:Location,w:WildlifeWeather) {
    time+=dt
    for(const a of animals) {
      if(!a.active) {
        a.wait-=dt
        if(a.wait<=0&&(w.shelter??0)<0.5&&(w.storm??0)<0.72) {
          spawn(a,p,w.direction);a.wait=12+rand()*12
        }
        continue
      }
      a.previousX=a.x;a.previousY=a.y+a.flown;a.previousZ=a.z;a.previousYaw=a.yaw
      a.age+=dt;a.timer-=dt;a.callAt-=dt;a.trackAt-=dt;a.avoidFor-=dt
      ...
      if(flee) {a.state='flee';a.timer=4.5;if(a.avoidFor<=0)a.targetYaw=Math.atan2(dx,dz)}
      else if(a.timer<=0) {
        a.state=rand()<0.44?'graze':'walk';a.timer=3+rand()*7
      ...
      if(bird&&a.state==='flee')a.flown=Math.min(5,a.flown+dt*1.4)
      else a.flown=Math.max(0,a.flown-dt*0.7)
      ...
      if(ny!==null&&ahead!==null&&Math.abs(ny-a.y)<0.24&&Math.abs(ahead-ny)<0.32) {
        ...
        a.speed=0
        if(a.avoidFor<=0){a.targetYaw=a.yaw+Math.PI*0.70;a.avoidFor=1.2}
      }
      if(a.callAt<=0) {
        if(a.state!=='flee'&&distance<42&&(w.storm??0)<0.65)habitat.onCall?.(a.kind,{x:a.x,y:a.y+0.6,z:a.z})
        a.callAt=18+rand()*25
      }
      if(a.speed>0.15&&a.trackAt<=0&&!bird) {
        habitat.onTrack?.(a.kind,{x:a.x,y:a.y,z:a.z},a.yaw);a.trackAt=(a.kind==='fox'?0.36:0.65)/a.speed
      }
      const behind=w.direction&&(dx*w.direction.x+dz*w.direction.z)<0
      if(distance>65||(a.age>100&&distance>25&&behind)) {
        a.active=false;a.model.root.visible=false;a.wait=25+rand()*35
      }
    }
  }
  return {root,animals,get time(){return time},update(dt:number,p:Location,w:WildlifeWeather={}){
    if(disposed||w.active===false)return
    if(w.reducedMotion){
      for(const a of animals)if(!a.active&&a.age===0){spawn(a,p,w.direction);a.age=1}
      for(const a of animals)if(a.active){a.model.root.position.set(a.x,a.y,a.z);a.model.root.rotation.y=a.yaw;a.model.update(0)}
      return
    }
    accumulator+=Math.max(0,Math.min(0.1,Number.isFinite(dt)?dt:0))
    while(accumulator+1e-8>=1/30){step(1/30,p,w);accumulator-=1/30}
    const blend=Math.max(0,Math.min(1,accumulator*30))
    for(const a of animals)if(a.active) {
      a.model.root.position.set(a.previousX+(a.x-a.previousX)*blend,
        a.previousY+(a.y+a.flown-a.previousY)*blend,a.previousZ+(a.z-a.previousZ)*blend)
  ...
  },dispose(){disposed=true;for(const a of animals)a.model.dispose();root.removeFromParent()}}
}
