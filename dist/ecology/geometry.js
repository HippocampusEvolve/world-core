...
export class Surface {
    ...
    loft(rings, paint, sides = 10) {
        const base = this.positions.length / 3;
        for (const [z, y, rx, ry] of rings) {
            for (let j = 0; j < sides; j++) {
                const a = j / sides * Math.PI * 2;
                const p = [Math.cos(a) * rx, y + Math.sin(a) * ry, z];
    ...
    ellipsoid(p, radii, c, sides = 8, rings = 5) {
    ...
    leaf(a, b, width, c, lift = 0.025) {
        ...
        const j = this.vertex(m.clone().add(side).toArray(), c);
        const k = this.vertex(b, c.clone().multiplyScalar(1.08));
        const l = this.vertex(m.clone().sub(side).toArray(), c.clone().multiplyScalar(0.85));
    ...
    append(g, paint) {
        ...
        if (g.index)
            for (const n of g.index.array)
                this.indices.push(n + base);
        else
