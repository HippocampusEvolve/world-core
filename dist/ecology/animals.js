...
export const ANIMAL_KINDS = ['fox', 'deer', 'raven'];
...
export function createAnimal(kind, seed = 1) {
    ...
    if (kind === 'raven') {
        ...
        for (let i = -2; i <= 2; i++)
            torso.leaf([i * 0.015, 0.25, -0.16], [i * 0.034, 0.22, -0.41], 0.023, coat);
        ...
        for (const side of [-1, 1]) {
            ...
            for (let toe = -1; toe <= 1; toe++)
                torso.branch([side * 0.055, 0.038, 0.035], [side * 0.055 + toe * 0.032, 0.012, 0.12 - Math.abs(toe) * 0.02], 0.008, 0.003, dark, 4);
        }
        const body = mesh(torso, 'raven-body');
        const wings = [-1, 1].map(side => {
            const pivot = new THREE.Group();
            pivot.position.set(side * 0.09, 0.33, 0);
            root.add(pivot);
            const wing = new Surface();
            wing.leaf([0, 0, 0.09], [side * 0.43, 0, -0.08], 0.15, coat, 0.028);
            for (let i = 0; i < 7; i++)
                wing.leaf([side * (0.14 + i * 0.045), 0, -0.04 - i * 0.012], [side * (0.23 + i * 0.057), -0.015, -0.31 + i * 0.025], 0.031, coat.clone().multiplyScalar(0.8 + i * 0.025));
            mesh(wing, 'raven-wing', pivot);
            return pivot;
        });
        return { root, kind, update(dt, pose = {}) {
                phase += Math.max(0, Math.min(dt, 0.1));
                const fly = pose.flying ?? 0;
                wings.forEach((w, i) => {
                    w.scale.set(fly ? 1 : 0.43, 1, fly ? 1 : 0.8);
            ...
            }, dispose() { for (const m of parts)
                m.geometry.dispose(); mat.dispose(); root.removeFromParent(); } };
    }
    const deer = kind === 'deer', legHeight = deer ? 0.92 : 0.36;
    const torso = new Surface();
    const bodyY = deer ? 1.12 : 0.49;
    const half = deer ? 0.62 : 0.36;
    const belly = (x, y, z) => y < bodyY - 0.085 && z > -half * 0.7
    ...
    neck.position.set(0, deer ? 1.35 : 0.59, deer ? 0.46 : 0.27);
    root.add(neck);
    ...
    for (const side of [-1, 1]) {
        const ey = deer ? [side * 0.113, 0.419, 0.174] : [side * 0.112, 0.135, 0.130];
        ...
        ear.scale(1, 1, 0.42);
        ear.rotateZ(-side * (deer ? 0.75 : 0.25));
        ear.translate(side * (deer ? 0.19 : 0.10), deer ? 0.61 : 0.28, deer ? 0.085 : 0.01);
        head.append(ear, (x, y, z) => z > 0.04 ? cream : paint(x, y, z));
        ear.dispose();
        if (deer && (seed & 1)) {
            const start = [side * 0.105, 0.52, -0.015], mid = [side * 0.24, 0.86, -0.14], end = [side * 0.39, 1.10, -0.04];
            head.branch(start, mid, 0.028, 0.018, color(0x8d816b));
            head.branch(mid, end, 0.018, 0.004, color(0xa5967d));
            for (let i = 0; i < 3; i++) {
                const t = i / 3, a = [side * (0.20 + t * 0.15), 0.80 + t * 0.24, -0.12];
    ...
    tailPivot.position.set(0, deer ? 1.28 : 0.54, -half);
    root.add(tailPivot);
    const tail = new Surface();
    tail.loft(deer ? [[-0.22, -0.12, 0.025, 0.022], [-0.14, -0.03, 0.07, 0.08], [0, 0, 0.045, 0.05]]
        : [[-0.59, -0.15, 0.008, 0.013], [-0.48, -0.14, 0.10, 0.095], [-0.28, -0.05, 0.12, 0.13], [-0.10, 0.025, 0.10, 0.10], [0, 0, 0.05, 0.065]], (x, y, z) => z < (deer ? -0.10 : -0.43) ? cream : paint(x, y, z), 10);
    ...
    for (const front of [false, true])
        for (const side of [-1, 1]) {
            ...
            mesh(up, `${kind}-upper-leg`, hip);
            mesh(low, `${kind}-foot`, shin);
