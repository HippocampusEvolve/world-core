...
export function logStack({ r = 0.055, len = 0.52, rows = [3, 2, 1], mats } = {}) {
    ...
    for (let row = 0; row < rows.length; row++) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            mesh.position.set(Math.sin(n * 7.3) * 0.02, row * r + (roll === Math.PI ? r : 0), (i - (count - 1) / 2) * STEP);
            g.add(mesh);
            n++;
        }
    }
...
//# sourceMappingURL=logstack.js.map