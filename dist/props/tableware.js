...
import { fillet, frameOf, merge, mesh, pipe, revolve } from './shapes.js';
/**
 ...
 */
export function mug({ fill = 0, mats } = {}) {
    ...
    const H = 0.1;
    const FLOOR = 0.004;
    const body = mesh('mug-body', revolve([
 ...
 */
export function sugarBowl({ mats } = {}) {
    const g = new THREE.Group();
    g.name = 'sugar-bowl';
    const m = roleMats(mats);
    const RIM = 0.078;
    const body = mesh('sugar-body', revolve([
    ...
    ], 12), [m('enamel'), m('paint2')]);
    g.add(body);
    const lid = mesh('sugar-lid', revolve([
        { pts: [[0, RIM], [0.051, RIM]] },
 ...
 */
export function cupShelf({ cups = 5, w = 0.7, mats } = {}) {
    const g = new THREE.Group();
    g.name = 'cup-shelf';
    const m = roleMats(mats);
    const D = 0.16;
...
function cup() {
    ...
    const at = (y) => 0.034 + ((y - 0.015) / 0.05) * 0.007 - 0.0005;
    const handle = pipe([
        [at(0.055), 0.055, 0],
        [0.056, 0.054, 0],
        [0.058, 0.035, 0],
...
//# sourceMappingURL=tableware.js.map