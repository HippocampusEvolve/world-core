 ...
 */
export function locker({ w = 0.5, d = 0.5, h = 1.8, open = false, sealed = false, hangers, mats } = {}) {
...
function hanger(rodY, rodZ, rodR) {
    const r = 0.0025;
 ...
 */
export function medCabinet({ open = true, mats } = {}) {
    const g = new THREE.Group();
    g.name = 'med-cabinet';
    const m = roleMats(mats);
    const w = 0.45;
    ...
    function vial() {
        const body = mesh('medcab-vial', revolve([
            [[0, 0], [0.016, 0]],
 ...
 */
export function panelBox({ w = 0.6, h = 0.8, d = 0.2, open = false, mats } = {}) {
    const g = new THREE.Group();
    g.name = 'panel-box';
    const m = roleMats(mats);
    const T = 0.012;
    ...
    for (const row of [0.12, -0.08]) {
        for (let i = 0; i < 12; i++)
            geos.push(boxGeo(bw, bh, bd, (i - 5.5) * bw, h / 2 + row, pz + PT / 2 + bd / 2));
    }
 ...
 */
export function jerrycan({ full = true, mats } = {}) {
    const g = new THREE.Group();
    g.name = 'jerrycan';
    const m = roleMats(mats);
    const W = 0.35;
    ...
    if (full)
        g.add(cylMesh('jerrycan-cap', 0.027, 0.027, 0.022, 12, m('steel'), 0.105, top + 0.03 + 0.011, 0, 4));
 ...
 */
export function bucket({ water = true, mats } = {}) {
    const g = new THREE.Group();
    g.name = 'bucket';
    const m = roleMats(mats);
    const S = 14;
    const wallAt = (y) => 0.1165 + (y - 0.018) * ((0.146 - 0.1165) / (0.3 - 0.018));
    g.add(mesh('bucket-body', revolve([
        [[0, 0.012], [0.114, 0.012]],
        [[0.114, 0.012], [0.114, 0]],
        [[0.114, 0], [0.12, 0]],
        [[0.12, 0], [0.15, 0.3]],
    ...
    for (const sx of [-1, 1])
        g.add(boxMesh('bucket-ear', 0.008, 0.035, 0.02, m('steel'), sx * (wall + 0.0018 + 0.004), ye, 0, 4));
