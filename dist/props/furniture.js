...
import { fillet, mesh, pipe, pivot } from './shapes.js';
/** Скамья: две доски сиденья на двух козлах, внизу проножка. Роли: wood, paint. */
export function bench({ w = 1.2, d = 0.35, h = 0.45, mats } = {}) {
    ...
    for (const sx of [-1, 1]) {
        ...
        for (const sz of [-1, 1])
            g.add(boxMesh('bench-leg', LEG, legH, LEG, m('paint'), x, legH / 2, sz * lz, 1, 'y'));
 ...
 */
export function chairStack({ n = 4, mats } = {}) {
    const g = new THREE.Group();
    g.name = 'chair-stack';
    const chairs = [];
    let one = null;
 ...
 * (ручки).
 */
export function desk({ w = 1.4, d = 0.7, h = 0.75, pull = 0, mats } = {}) {
    const g = new THREE.Group();
    g.name = 'desk';
    const m = roleMats(mats);
    const TOP = 0.03;
 ...
 */
export function nightstand({ w = 0.45, d = 0.4, h = 0.6, pull = 0, mats } = {}) {
    const g = new THREE.Group();
    g.name = 'nightstand';
    const m = roleMats(mats);
    const TOP = 0.02;
 ...
 */
export function sideboard({ w = 1.4, d = 0.45, h = 0.9, open = 0, mats } = {}) {
    const g = new THREE.Group();
    g.name = 'sideboard';
    const m = roleMats(mats);
    const TOP = 0.025;
 ...
 */
export function workbench({ w = 1.6, d = 0.6, h = 0.9, mats } = {}) {
    const g = new THREE.Group();
    g.name = 'workbench';
    const m = roleMats(mats);
    const TOP = 0.05;
    ...
    for (const sx of [-1, 1])
        for (const sz of [-1, 1])
            g.add(boxMesh('workbench-leg', LEG, under, LEG, m('paint'), sx * lx, under / 2, sz * lz, 1, 'y'));
    const AP = 0.08;
    const AT = 0.025;
    const inX = 2 * lx - LEG;
    const inZ = 2 * lz - LEG;
    ...
    for (const sx of [-1, 1])
        g.add(boxMesh('workbench-apron', AT, AP, inZ, m('paint'), sx * lx, under - AP / 2, 0, 1, 'z'));
 ...
 */
export function crate({ w = 0.9, d = 0.6, h = 0.6, open = 0, mats } = {}) {
    const g = new THREE.Group();
    g.name = 'crate';
    const m = roleMats(mats);
    const wood = m('wood');
    ...
    for (let i = 0; i < 3; i++) {
        ...
        for (const sz of [-1, 1])
            g.add(boxMesh('crate-board', w, rowH, B, wood, 0, y, sz * (d / 2 - B / 2), 2, 'x'));
        for (const sx of [-1, 1])
            g.add(boxMesh('crate-board', B, rowH, d - 2 * B, wood, sx * (w / 2 - B / 2), y, 0, 2, 'z'));
    }
    ...
    for (const sx of [-1, 1])
        lid.add(boxMesh('crate-cleat', 0.07, CL, d - 0.04, wood, sx * (w / 2 - 0.1), B + CL / 2, d / 2, 2, 'z'));
    lid.rotation.x = -open;
    g.add(lid);
