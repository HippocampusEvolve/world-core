...
export function generator({ l = 1.5, d = 0.8, h = 1.1, plinth = 0.1, mats } = {}) {
    ...
    const X = (v) => v * sx;
    const Z = (v) => v * sz;
    const Y = (v) => plinth + (v - 0.1) * sy;
    const span = (x0, x1, y0, y1, z0, z1) => spanGeo(X(x0), X(x1), Y(y0), Y(y1), Z(z0), Z(z1));
    const add = (name, geo, role) => g.add(part(`generator-${name}`, geo, look(role)));
    // постамент и салазки
    add('plinth', spanGeo(-l / 2, l / 2, 0, plinth, -d / 2, d / 2), 'paint2');
    g.add(merged('generator-skids', [-1, 1].map((s) => span(-0.66, 0.74, 0.1, 0.16, s * 0.26 - 0.04, s * 0.26 + 0.04)), look('paint2')));
    ...
    const inner = [cylGeo(HUB, HUB, X(0.083), 16, 'x', X(-0.745) - fx + X(0.083) / 2, 0, 0)];
    for (let i = 0; i < 5; i++) {
