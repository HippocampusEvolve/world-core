...
function cradle(hw, y0, r, yc, seg) {
...
function saddleTop(hw, y1, r, yc, seg) {
    const a = Math.asin(hw / r);
    const pts = [];
    for (let i = 0; i <= seg; i++) {
        const t = -a + (2 * a * i) / seg;
...
function revolveX(profile, seg, x0, y, z) {
    const g = revolveGeo(profile, seg);
    g.rotateZ(-Math.PI / 2); // локальная +Y уходит в +X
    g.translate(x0, y, z);
    return g;
}
export function pump({ w = 1.2, d = 0.6, h = 0.8, mats } = {}) {
    ...
    const circ = (a) => [RC * Math.cos(a), yc + RC * Math.sin(a)];
    const fins = [];
    for (let i = 0; i < 9; i++)
        fins.push((-60 + i * 15) * deg);
    for (let i = 0; i < 9; i++)
        fins.push((120 + i * 15) * deg);
    const dl = Math.asin(FT / 2 / RC);
    const motor = [];
    const arcTo = (a0, a1) => {
        const n = Math.max(1, Math.round((a1 - a0) / (10 * deg)));
        for (let i = 1; i < n; i++)
            motor.push(circ(a0 + ((a1 - a0) * i) / n));
    };
    let at = -90 * deg;
    motor.push(circ(at));
    for (const a of fins) {
        ...
        const tip = (s) => [(RC + FIN) * u[0] + s * (FT / 2) * v[0], yc + (RC + FIN) * u[1] + s * (FT / 2) * v[1]];
    ...
    g.add(part('pump-cowl', revolveX([
        ...
        [RF, CL],
        [0, CL],
    ], 24, MX0 - CL, yc, 0), look('paint')));
    const SH = 0.035 * sx;
    g.add(part('pump-shield', revolveX([
        [0, 0],
        [RC - 0.01 * k, 0],
        [RC - 0.01 * k, 0.012 * sx],
        [0.09 * k, 0.028 * sx],
    ...
    const flanges = [cylGeo(FR, FR, FL, 24, 'x', SX1 + FL / 2, yc, 0)];
    const nuts = [];
    const NUT = 0.01 * k;
    for (let i = 0; i < 4; i++) {
        const a = (i / 4) * Math.PI * 2 + Math.PI / 4;
