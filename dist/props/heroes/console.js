...
const SLAB = 0.03;
const HOOK = 0.09;
export function listeningConsole({ w = 2.8, d = 0.7, h = 1.1, mats } = {}) {
    ...
    // лицо и кромка не накрывают друг друга, поэтому и не спорят
    const DR = 0.055;
    const DH = 0.014;
    const FR = 0.046;
    const FH = 0.008;
    ...
    XS.forEach((x, i) => {
        ...
        n.add(merged(`needle-${i + 1}`, [spanGeo(-0.001, 0.001, 0.0005, 0.002, -0.036, 0.006), cylGeo(0.004, 0.004, 0.003, 6, 'y', 0, 0.002, 0)], look('plastic')));
        panel.add(n);
        needles.push(n);
    });
    ...
    sel.add(merged('selector', [cylGeo(0.02, 0.02, 0.018, 12, 'y', 0, 0.009, 0), spanGeo(-0.003, 0.003, 0.018, 0.024, -0.028, 0)], look('plastic')));
    panel.add(sel);
    ...
    const s0 = SS - SH / 2;
    const s1 = SS + SH / 2;
    panel.add(merged('console-screen-frame', [
    ...
    const HZ = 0.1;
    const HR = 0.006;
    g.add(merged('console-hook', [cylGeo(HR, HR, 0.085, 8, 'x', xr + 0.0425, HY, HZ), cylGeo(0.005, 0.005, 0.019, 8, 'y', xr + 0.08, HY + HR + 0.0095, HZ)], look('steel')));
    ...
    const at = (x, y, s) => pointOf(g, panel, new THREE.Vector3(x, y, -s));
    return {
        group: g,
        w,
        d,
