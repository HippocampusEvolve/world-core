 ...
 */
export function pivot(name, x, y, z, axis, range, at = 0, spin = false) {
    const g = new THREE.Group();
    g.name = name;
    g.position.set(x, y, z);
    const info = { moving: name, axis, range, spin };
    g.userData = info;
    turn(g, at);
    return g;
}
/** Все подвижные части предмета по именам. */
export function movingParts(root) {
    const out = {};
    root.traverse((o) => {
        const name = o.userData.moving;
        if (name)
            out[name] = o;
    });
    return out;
...
export function pointOf(root, o, local = new THREE.Vector3()) {
 ...
 * ------------------------------------------------------------------------ */
/** Меш с именем. */
export function part(name, geo, mat) {
...
/** Коробка по границам. */
export function spanGeo(x0, x1, y0, y1, z0, z1) {
 ...
 */
export function cylGeo(r0, r1, len, seg, axis, x, y, z) {
...
export function torusGeo(R, r, radial, tubular, axis, x, y, z, arc = Math.PI * 2) {
    const g = new THREE.TorusGeometry(R, r, radial, tubular, arc);
    const uv = g.attributes.uv;
    for (let i = 0; i < uv.count; i++)
        uv.setXY(i, uv.getX(i) * arc * R, uv.getY(i) * Math.PI * 2 * r);
 ...
 */
export function revolveGeo(profile, seg, smooth = false, matOf = () => 0) {
    const onAxis = (p) => Math.abs(p[0]) < 1e-9;
    ...
    for (let j = 0; j + 1 < pts.length; j++)
        links.push([j, j + 1]);
    if (closed)
        links.push([pts.length - 1, 0]);
    // длина вдоль профиля - для V
    const along = [0];
    for (let j = 1; j < pts.length; j++)
        along.push(along[j - 1] + Math.hypot(pts[j][0] - pts[j - 1][0], pts[j][1] - pts[j - 1][1]));
    const ring = (p, v) => {
    ...
    const fan = (j, axisY, first) => {
        ...
        for (let i = 0; i < seg; i++) {
            if (first)
                idx.push(r + i + 1, r + i, c);
            else
                idx.push(r + i, r + i + 1, c);
        }
    };
    if (startCap) {
        use(matOf(0));
...
function shapeOf(outline, holes = []) {
    ...
    for (const h of holes)
        s.holes.push(new THREE.Path(h.map(([a, b]) => new THREE.Vector2(a, b))));
    return s;
}
/**
 ...
 * координат.
 */
export function prismGeo(outline, axis, from, to, holes = []) {
    const depth = to - from;
...
export function arc(cx, cy, r, a0, a1, seg) {
    const out = [];
    for (let i = 0; i <= seg; i++) {
 ...
 */
export function wireGeo(points, r, segments, radial = 4, smooth = true) {
    let curve;
    if (smooth)
    ...
    const uv = g.attributes.uv;
    for (let i = 0; i < uv.count; i++)
        uv.setXY(i, uv.getX(i) * len, uv.getY(i) * Math.PI * 2 * r);
    return g;
}
/**
 ...
 */
export function merged(name, geos, mat) {
    ...
    if (!out)
        throw new Error(`merged(${name}): детали с разными атрибутами`);
    for (const g of geos)
        g.dispose();
