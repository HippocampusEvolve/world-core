...
export function radio({ w = 0.5, d = 0.25, h = 0.3, mats } = {}) {
    ...
    const GY0 = 0.14 * sy;
    const GY1 = h - 0.035;
    const grille = roundRect(2 * GX, GY1 - GY0, 0.02, 3).map(([x, y]) => [x, y + (GY0 + GY1) / 2]);
    const DX = w / 2 - 0.08;
    const DY0 = 0.085 * sy;
    const DY1 = 0.12 * sy;
    const win = [
    ...
    [-KX, 0, KX].forEach((x, i) => {
        ...
        k.add(merged(`knob-${i + 1}`, [cylGeo(0.021, 0.021, 0.008, 18, 'z', 0, 0, 0.004), cylGeo(0.017, 0.015, 0.014, 18, 'z', 0, 0, 0.015)], look('plastic')));
