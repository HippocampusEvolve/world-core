 ...
 */
export function splitLogGeometry(r, len, span = Math.PI, segments = 8, s = {}) {
    const sb = s.bark ?? 2.6;
    ...
    for (const side of [1, -1]) {
        ...
        for (let i = 0; i < segments; i++) {
            if (side > 0)
                idx.push(center, ring[i + 1], ring[i]);
            else
    ...
    const inside = [r * 0.5, 0];
    const flats = span >= Math.PI - 1e-6 ? [[at(a0), at(a0 + span)]] : [[[0, 0], at(a0)], [at(a0 + span), [0, 0]]];
    ...
    g.setIndex(idx);
    for (const [st, count, mat] of groups)
        g.addGroup(st, count, mat);
    return g;
}
/**
 ...
 */
export function roundLogGeometry(r, len, segments = 8, s = 2.6) {
...
export function logMaterials(mats) {
    return {
