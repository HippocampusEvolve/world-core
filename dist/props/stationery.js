 ...
 */
export function openBook({ w = 0.42, d = 0.3, thick = 0.012, mats } = {}) {
    ...
    // чуть проседает
    const top = (u) => thick * (0.3 + 0.7 * Math.sin(Math.min(1, u / 0.4) * (Math.PI / 2))) - thick * 0.06 * Math.max(0, u - 0.8) / 0.2;
    const outline = [[XS, 0], [xf, 0]];
    const K = 10;
    for (let k = K; k >= 0; k--) {
        const u = k / K;
    ...
    const block = (name, side) => {
        const pts = side > 0 ? outline : outline.map(([x, y]) => [-x, y]).reverse();
    ...
    const parts = [boxGeo(2 * XS, TC, d, 0, TC / 2, 0)];
    for (const s of [-1, 1]) {
        parts.push(boxGeo(OV, TC, d, s * (xf + OV / 2), TC / 2, 0));
        for (const e of [-1, 1])
            parts.push(boxGeo(xf - XS, TC, OV, s * (XS + xf) / 2, TC / 2, e * (dp / 2 + OV / 2)));
    }
